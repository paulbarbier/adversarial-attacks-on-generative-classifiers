...
from models.Log_p_x_z import Log_p_x_z
from models.Log_p_y_xz import Log_p_y_xz
...
class ClassifierDFZ(nn.Module):
    ...
    def __call__(self, X, y, epsilon, train: bool = False): # X: (height, width), y: (n_classes,), epsilon: (d_latent,) -> 1, 1
        ...
        logit_p_x_z = Log_p_x_z(
            n_classes=self.n_classes,
            d_latent=self.d_latent,
            d_hidden=self.d_hidden,
            dropout_rate=self.dropout_rate,
        )(X, z, train)
        logit_p_y_xz = Log_p_y_xz(
            n_classes=self.n_classes,
            d_latent=self.d_latent,
            d_hidden=self.d_hidden,
            dropout_rate=self.dropout_rate,
...
def create_and_init(key, config: ConfigDict, dataset_config: ConfigDict) -> ClassifierDFZ:
    model = ClassifierDFZ(
       **config.model,
...
def log_likelyhood_A(z, logit_q_z_xy, logit_p_x_z, logit_p_y_xz):
